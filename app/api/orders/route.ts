...
export async function POST(req: Request) {
  ...
  // Queue tickets are free to create: walk-ups and cash pre-orders join the
  // queue immediately (pay at the counter). Card pre-orders go through
  // Stripe (preferred) or Mollie when configured, joining the queue on payment.
  const onlineConfigured = isStripeConfigured() || isMollieConfigured()
  const wantsPayment = lineItems.length > 0 && payMethod === 'card' && onlineConfigured
