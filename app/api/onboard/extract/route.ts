...
export async function POST(req: Request) {
  ...
  if (!image || image.length > MAX_BASE64_LENGTH || !ALLOWED_MEDIA_TYPES.includes(mediaType)) {
    return NextResponse.json(
      { error: 'Please upload a JPEG, PNG or WebP photo under 5MB.' },
      { status: 400 }
    )
  }
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'text',
              text: 'This is a photo of a food vendor\'s menu. Extract every menu item you can read: name, description (empty string if none printed), price in pence (e.g. £5.50 → 550), and the menu section it belongs to. Keep the original wording and item order.',
            },
          ],
        },
      ],
